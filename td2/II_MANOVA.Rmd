...
```{r}
# à compléter 
data_csv <- read.csv("MANOVA_DATASET.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
```{r}
# à compléter 
save(data_csv, file = "MANOVA_DATASET.Rda")
...
```{r,echo = T,  warning = F}
# à compléter 
data_csv$Group <- factor(data_csv$Site)
data_csv$Site <- NULL

table(data_csv$Group)
nlevels(data_csv$Group)
...
```{r P}
# à compléter  
id_num <- which(sapply(data, is.numeric))
P <- length(id_num)
P
...
data_csv[, c("Al","Fe","Mg","Ca","Na")] <- lapply(data_csv[, c("Al","Fe","Mg","Ca","Na")], as.numeric)
data_csv$Group <- factor(data_csv$Group)

# check indispensable
print(table(data_csv$Group))

fit <- manova(cbind(Al, Fe, Mg, Ca, Na) ~ Group, data = data_csv)
...
```{r, echo = T}
# à compléter   
fit_iris <- manova(as.matrix(iris[, 1:4]) ~ Species, data = iris)
summary(fit_iris, test = "Wilks")
